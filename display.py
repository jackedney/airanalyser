...
class DisplayManager:
    def __init__(self, device):
        ...
        try:
            ...
            self.small_font = ImageFont.truetype(
                "/usr/share/fonts/truetype/roboto/Roboto-Regular.ttf", 12
            )
        except Exception:
    ...
    def _draw_horizontal_gauge(
        ...
        draw.text((x + width - poor_w, y + height + 6), "Poor", font=self.small_font, fill=1)
    ...
    def _draw_reading_page(
        ...
        draw.text(
            (64 - unit_w / 2, 80), ranges["unit"], font=self.small_font, fill=1
        )

        # Draw the gauge lower on the screen with more space above
        self._draw_horizontal_gauge(
            draw, 10, 95, 108, 10, value, indicator_type
        )
